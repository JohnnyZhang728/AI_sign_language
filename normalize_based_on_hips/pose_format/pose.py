...
class Pose:
    ...
    def normalize(self, info: PoseNormalizationInfo, scale_factor: float = 1) -> "Pose":
        ...
        p1s = transposed[info.p1] # RIGHT_HIP
        p2s = transposed[info.p2] # LEFT_HIP
        # (Points, People, Frames, Dims)
        # print("P1S_SHAPE:", p1s.shape)
        # p1s.shape: (1, 88, 3) (People, Frames, Dims)

        center = ((p2s + p1s) / 2).mean(axis=(0, 1))
        ## .mean(axis=(0, 1)):: for every person and for every frame calculate average::  dim_0:people; dim_1:frame
        # print("CENTER:", center)
        
        # print("self.body.data-ORIGINAL:", self.body.data)

        #self.body.data -= center
        # ONLY move (x,y)
        self.body.data[..., :2] -= center[:2]
        # print("self.body.data-sub-CENTER:", self.body.data)

        # mean_distance = distance_batch(p1s, p2s).mean()
        # ONLY (x,y)
        mean_distance = distance_batch(p1s[..., :2], p2s[..., :2]).mean()
        ## distance_batch(p1s, p2s) calculates the Euclidean distance between p1s and p2s and returns a matrix of (number of people, number of frames).
        ...
        # ONLY scale (x,y)
        self.body.data[..., :2] *= scale
        # print("self.body.data-AFTER:", self.body.data)
